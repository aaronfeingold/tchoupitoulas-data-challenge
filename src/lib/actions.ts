...
export async function getMonthlyTotals(year?: number) {
  try {
    const yearFilter = year
      ? and(
          gte(hallOfFameEntries.parsedDate, startOfYear(new Date(year, 0, 1))),
...
export async function getDailyTotalsForMonth(year: number, month: number) {
  try {
...
export async function getMostCommonNames() {
  try {
    const result = await db
      .select({
...
export async function getEntriesForName(name: string) {
  try {
    const entries = await db
      .select()
      .from(hallOfFameEntries)
...
export async function getLongestGap() {
  try {
